<template>
  <section class="container mx-auto py-0">
    <section class="flex flex-col xl:flex-row py-20 gap-10 items-center overflow-hidden xl:pr-10 lg:rounded-r-3xl">
      ...
      <div class="flex flex-col relative z-0 shrink-0">
        <!-- Extra large yellow circle background - hidden on mobile, visible on lg -->
        <div class="absolute border-2 border-secondary/50 inset-0 -inset-x-32 -inset-y-32 -top-48 -left-48 lg:-inset-x-72 lg:-inset-y-72 lg:-top-96 lg:-left-96 bg-secondary/30 rounded-full"></div>
        <!-- Large yellow circle background -->
        <div class="absolute border-2 border-secondary/50 inset-0 -inset-x-16 -inset-y-16 -left-20 -top-20 lg:inset-5 lg:-inset-x-24 lg:-inset-y-24 lg:-left-40 lg:-top-40 bg-secondary/50 rounded-full"></div>
        
        <button 
          ref="buttonRef"
          class="flex flex-row items-center bg-secondary hover:bg-base-100 text-base-content transition-all duration-300 hover:cursor-pointer h-fit rounded-full py-16 px-6 sm:py-24 sm:px-8 lg:py-36 lg:px-10 whitespace-nowrap font-poppins font-bold text-xl sm:text-2xl lg:text-4xl border-2 border-base-content overflow-hidden relative z-10"
        >
          Get Started
...
import ArrowDiagonal from "./ArrowDiagonal.vue"
